...
class Imap
{
	...
	*/
	...
	public function __construct(array $config)
    {
        // Ensure that the necessary configuration keys are present
        if (!isset($config['hostname'], $config['username'], $config['password'], $config['port'], $config['mailbox'])) {
            throw new InvalidArgumentException('Invalid IMAP configuration');
        }

        // Create the IMAP connection using the provided configuration
        $this->stream = imap_open(
            '{' . $config['hostname'] . ':' . $config['port'] . '/' . $config['protocol'] . '}' . $config['mailbox'],
            $config['username'],
            $config['password']
        );

        if (!$this->stream) {
            throw new RuntimeException('Unable to connect to the IMAP server');
        }
    }
	...
	public function connect(array $config = [])
	{
		...
		$this->stream  = imap_open($this->mailbox, $config['protrack@mdsengg.com'], $config['Protrack']);
	...
	{
		 // Check if $uid is a valid UID and greater than 0
		if ($uid <= 0) 
		{
			return false;
		}
	...
	public function get_thread(int $uid)
    {
		
        // Fetch the original message
        $originalMessage = $this->get_message($uid);

        // Fetch all related messages (replies)
        $relatedMessages = $this->get_messages($originalMessage['in_reply_to']);

        // Combine the original message and related messages to form the thread
        $thread = [$originalMessage];

        foreach ($relatedMessages as $relatedMessage) {
            $thread[] = $relatedMessage;
        }
